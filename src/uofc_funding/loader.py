...
class DataLoader:
    ...
    def _load_round_sheet(
        ...
        try:
            ...
            warnings.append(ValidationWarning(
                severity="error",
                message=f"Failed to read sheet '{sheet_name}'",
                details=str(e)
            ))
            return None, warnings
    ...
    def _load_overview_sheet(
        ...
        try:
            ...
            warnings.append(ValidationWarning(
                severity="warning",
                message=f"Failed to read overview sheet",
                details=str(e)
            ))
            return None, warnings
        ...
        # Try to identify the standard overview columns
        overview_cols = self.config.overview_columns
        
        # Look for year columns (2023-2024, 2024-2025, etc.)
        year_pattern_cols = [c for c in df.columns if "-" in str(c) and any(
            str(y) in str(c) for y in range(2020, 2030)
        )]
    ...
    def _validate_data_quality(self, df: pd.DataFrame) -> list[ValidationWarning]:
        ...
        for col in key_columns:
            if col in df.columns:
                ...
                if missing_pct > 0:
                    warnings.append(ValidationWarning(
                        severity="warning" if missing_pct < self.config.validation.max_missing_percent else "error",
    ...
    def _convert_numeric_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        numeric_columns = [
            "amount_requested", "amount_awarded", "funding_ratio",
            "current_funds", "estimated_income", "active_members", "panlist_size"
        ]
