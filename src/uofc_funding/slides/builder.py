...
class PresentationBuilder:
    ...
    def _add_title_slide(self) -> None:
        ...
        title_box = slide.shapes.add_textbox(
            Inches(0.5), Inches(4.0), Inches(9), Inches(1.0)
        )
        ...
        link_box = slide.shapes.add_textbox(
            Inches(0.5), Inches(5.0), Inches(9), Inches(1.0)
        )
    ...
    def _add_age_group_slide(self) -> None:
        ...
        if age_csv.exists():
            self._add_table_from_csv(
                slide, age_csv,
                self.layouts.table_left.offset(left=-0.3, top=-0.5)
            )
        ...
        if avg_csv.exists():
            self._add_table_from_csv(
                slide, avg_csv,
                self.layouts.table_right.offset(left=0.3, top=-0.5)
            )
    ...
    def _add_other_funding_comparison_slide(self) -> None:
        ...
        if bar_path.exists():
            self._add_scaled_image(
                slide, bar_path,
                self.layouts.split_chart.offset(top=0.7)
            )
        ...
        if csv_path.exists():
            self._add_table_from_csv(
                slide, csv_path,
                self.layouts.split_table.offset(top=0.7, height=-2)
            )
