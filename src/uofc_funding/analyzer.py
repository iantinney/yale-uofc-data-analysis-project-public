...
class FundingAnalyzer:
    ...
    def _compute_avg_by_category(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        grouped = df.groupby("organization_category")[
    ...
    def _compute_avg_by_age(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        grouped = df.groupby("organization_age")[
            ["amount_requested", "amount_awarded"]
        ].mean().round(2)
    ...
    def _compute_dwight_hall_counts(self, df: pd.DataFrame) -> pd.Series:
        ...
        df["is_dwight_hall"] = (
            df["other_funding"]
            .astype(str)
            .str.contains("Dwight Hall", case=False, na=False)
        )
    ...
    def _compute_avg_by_dwight_hall(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        grouped = df.groupby("is_dwight_hall")[
            ["amount_requested", "amount_awarded"]
        ].mean().round(2)
    ...
    def _compute_avg_by_dwight_hall_column(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df = df.copy()
        df["is_dwight_hall"] = df["dwight_hall"].astype(str).str.lower().isin(
            ["yes", "y", "true", "1"]
        )
    ...
    def _compute_avg_by_other_funding(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        grouped = df.groupby("has_other_funding")[
            ["amount_requested", "amount_awarded"]
        ].mean().round(2)
    ...
    def _compute_top_organizations(
        self, 
        df: pd.DataFrame, 
        top_n: int = 5
    ) -> pd.DataFrame:
        ...
        top["funding_ratio_pct"] = (
            (top["amount_awarded"] / top["amount_requested"]) * 100
        ).round(2)
